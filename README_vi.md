# Prompt cho ChatGPT Images 2.5 — flaq.ai

Dự án mã nguồn mở độc lập do nhóm [flaq.ai](https://flaq.ai) xây dựng và duy trì, dành cho ảnh sản phẩm, chỉnh sửa có chọn lọc, chân dung, quảng cáo, đồ họa thông tin và bảng phân cảnh.

[English](README.md) · [简体中文](README_zh.md) · [繁體中文](README_tw.md) · [日本語](README_ja.md) · [한국어](README_ko.md) · [Español](README_es.md) · [Français](README_fr.md) · [Deutsch](README_de.md) · [Português (Brasil)](README_pt.md) · [Italiano](README_it.md) · [Русский](README_ru.md) · [العربية](README_ar.md) · [हिन्दी](README_hi.md) · [ไทย](README_th.md) · [Bahasa Indonesia](README_id.md) · **Tiếng Việt**

![flaq.ai — ChatGPT Images 2.5](assets/images/cover.png)

Thư viện có 103 công thức trong 15 nhóm và 139 ảnh mới: 73 công thức đầy đủ bằng tiếng Anh và tiếng Trung giản thể, cùng 12 công thức riêng theo ngôn ngữ. README có 16 phiên bản ngôn ngữ và khu vực. Trang này là phần giới thiệu bằng tiếng Việt, không phải bản dịch toàn bộ thư viện. Ngoài ra còn có 18 công thức quy trình mới chỉ bằng tiếng Anh.


**[Thư viện ảnh cho 103 công thức](docs/gallery.md)**

## Bắt đầu

Chọn công thức trong [mục lục](prompts/README.md). Khi chỉnh sửa, đính kèm các ảnh tham chiếu được chỉ định và phân biệt phần cần thay đổi với phần phải giữ nguyên. Kiểm tra kết quả rồi dùng ảnh đã duyệt làm đầu vào cho lần chỉnh sửa tiếp theo.

## Thử bằng tiếng Việt

[↗ L005](prompts/11-multilingual.md#l005)

```text
Tạo áp phích dọc 2:3 cho một quán cà phê hư cấu. Trên nền giấy màu kem, đặt một tách gốm xanh chứa cà phê và một thìa nhỏ, với ánh sáng buổi sáng chiếu từ bên cạnh. Phía trên ghi chính xác "CÀ PHÊ, CHẲNG VỘI", bên dưới là "Dành một chút thời gian" và cuối áp phích là "CÀ PHÊ KHU PHỐ". Dùng chữ xanh đậm, lề rộng và thứ bậc chữ rõ ràng. Giữ đầy đủ dấu tiếng Việt và dấu phẩy; không thêm giá hoặc địa chỉ. Ở lần chỉnh sửa tiếp theo, chỉ đổi màu tách sang xanh ô liu, giữ nguyên chữ, bóng đổ và bố cục.
```

Kiểm tra dấu thanh, dấu phụ, chữ PHỐ và khoảng cách dòng để dấu không bị cắt.

## Tình trạng ví dụ

Prompt dưới đây là bản điều chỉnh từ L005, không được tính là công thức mới và chưa có ảnh kết quả. Ảnh của dự án được tạo bằng công cụ tích hợp trong Codex, không trả về mã mô hình. Đây không phải phép so sánh đã xác minh giữa Flare và Sunburst. Hãy kiểm tra chữ, số lượng vật thể và hình dạng trước khi đăng.

## Về flaq.ai

[flaq.ai](https://flaq.ai) cung cấp API hợp nhất cho các mô hình hình ảnh, video, âm nhạc và ngôn ngữ, phục vụ người phát triển tác nhân AI và ứng dụng. Nhóm chia sẻ thư viện này để cung cấp các prompt có thể tái sử dụng cùng ghi chú đánh giá kết quả.

Các liên kết Flaq.ai dưới đây dành cho GPT Image 2. Để dùng Images 2.5 trực tiếp qua OpenAI, xem [hướng dẫn API](docs/api-guide.md).

[GPT Image 2 API](https://flaq.ai/models/openai/gpt-image-2/) · [GPT Image 2 Edit API](https://flaq.ai/models/openai/gpt-image-2-edit/) · [API docs](https://flaq.ai/docs/) · [Model market](https://flaq.ai/model-market/) · [Affiliate program](https://flaq.ai/affiliate-program/)

## Tài liệu và đóng góp

[103 prompts](prompts/README.md) · [Before / after](docs/launch-examples.md) · [Generation log](docs/generation-log.md) · [OpenAI API guide](docs/api-guide.md) · [Contributing](CONTRIBUTING.md)

Dự án không liên kết với OpenAI và không được OpenAI bảo trợ. [MIT License](LICENSE) © 2026 Flaq AI.

[X community: 6 English prompts, original posts and source image previews](prompts/15-x-community.md)
