...
当前版本包含 **72 条原创配方、11 个场景包**：60 条完整中英双语核心提示词，以及12条本地语言配方。翻译、追问和变体不重复计数。另有 **7 张本次新生成图片：1张品牌封面 + 6张场景及编辑示例**。未配图条目标明“尚未实测”，不把全部配方描述成已验证案例。
...
没有。当前是60条双语核心配方、12条本地语言配方和7张新生成图片。各条目清楚标识配图状态，修改指令也不一概声称已执行。
