...
当前版本包含 **103 条配方、15 个场景包**：73 条完整中英双语核心提示词、18条英文工作流配方，以及12条本地语言配方。翻译、追问和变体不重复计数。**全部103条现已配图**，共139张原创PNG，包含生成结果、编辑参考图、修改过程与封面。本次补齐85条缺图配方，并为91条核心提示词补充适用场景、输入要求和交付检查。可直接浏览[完整图片画廊](docs/gallery.md)。
...
全部103条已有对应生成示例，其中73条双语核心配方、18条英文工作流配方、12条本地语言配方。配图不代表所有要求均完美达成；每张图附有实际效果说明，建议中的后续修改只有单独记录结果时才表示已执行。
